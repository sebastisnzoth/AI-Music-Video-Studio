...
app = FastAPI(title="AI Music Video Studio Control", version="0.12.0")
...
def _worker_health() -> dict:
    ...
    try:
        req = urllib.request.Request(f"{base}/api/health", headers={"User-Agent": "AI-Music-Video-Studio-Vercel/0.12"})
...
<div class="layout"><section class="card"><h2>Nuevo videoclip</h2><form id="form" class="grid"><div><label>Canción</label><input type="file" name="song" accept="audio/*" required></div><div><label>Foto o video</label><input type="file" name="visual" accept="image/*,video/*" required></div><div><label>Título</label><input name="title" placeholder="Mi videoclip"></div><div><label>Estilo</label><select name="style"><option value="cinematic rock">Cinematic Rock</option><option value="romantic film">Romántico</option><option value="urban night">Urbano nocturno</option><option value="live performance">Performance</option><option value="dreamlike surreal">Surreal / Dream</option></select></div><div><label>Formato</label><select name="aspect"><option>16:9</option><option>9:16</option></select></div><div><label>Calidad</label><select name="quality"><option value="preview">Preview</option><option value="final" selected>Final 1080p</option><option value="master">Master</option></select></div><div class="full"><label>Letra (opcional)</label><textarea name="lyrics" placeholder="Pegá la letra..."></textarea></div><div class="full"><label>Checkpoint ComfyUI</label><input id="checkpoint" placeholder="modelo.safetensors"></div><div class="full"><button id="createBtn" class="primary" {'disabled' if not configured else ''}>Crear proyecto en worker</button></div></form><div id="status" class="status">{'Listo para conectar al worker.' if configured else 'Configurá RENDER_WORKER_URL en Vercel.'}</div><div id="preview" class="preview"></div></section><section class="card"><h2>Storyboard</h2><div id="storyboard" class="storyboard"><p class="muted">Creá un proyecto para ver las escenas.</p></div></section></div></main>
<script>
const WORKER={worker_js};const f=document.querySelector('#form'),s=document.querySelector('#status'),sb=document.querySelector('#storyboard'),pv=document.querySelector('#preview'),cp=document.querySelector('#checkpoint'),badge=document.querySelector('#workerBadge');let current=null;const running=new Set();
async function health(){{try{{const j=await fetch('/api/control/health').then(r=>r.json());badge.textContent='Worker: '+(j.online?'online':'offline');}}catch{{badge.textContent='Worker: error'}}}}health();
