...
app = FastAPI(title="AI Music Video Studio", version="0.3.0")
...
function draw(rows){sb.innerHTML=rows.map(x=>`<div class="scene"><b>ESCENA ${x.id} · ${x.start}s–${x.end}s</b><div class="energy">energía: ${x.energy_band||x.energy||'medium'} · ${x.generation_engine||'reference'}</div><div class="director">${x.strategy||'narrative'} ${x.needs_lipsync?'· lip-sync':''}</div><p>${esc(x.lyrics||x.director_prompt||x.prompt)}</p><button onclick="approve(${x.id},${!x.approved})">${x.approved?'Quitar OK':'Aprobar'}</button></div>`).join('')}
