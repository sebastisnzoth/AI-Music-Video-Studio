...
app = FastAPI(title="AI Music Video Studio Control", version="0.12.2")
...
def _worker_json(path: str, timeout: float = 4.0) -> dict:
    ...
    req = urllib.request.Request(
        f"{base}/{path.lstrip('/')}",
        headers={"User-Agent": "AI-Music-Video-Studio-Vercel/0.12.2"},
    )
...
<div class="layout"><section class="card"><h2>Nuevo videoclip</h2><form id="form" class="grid"><div><label>Canción</label><input type="file" name="song" accept="audio/*" required></div><div><label>Foto o video</label><input type="file" name="visual" accept="image/*,video/*" required></div><div><label>Título</label><input name="title" placeholder="Mi videoclip"></div><div><label>Estilo</label><select name="style"><option value="cinematic rock">Cinematic Rock</option><option value="romantic film">Romántico</option><option value="urban night">Urbano nocturno</option><option value="live performance">Performance</option><option value="dreamlike surreal">Surreal / Dream</option></select></div><div><label>Formato</label><select name="aspect"><option>16:9</option><option>9:16</option></select></div><div><label>Calidad</label><select name="quality"><option value="preview">Preview</option><option value="final" selected>Final 1080p</option><option value="master">Master</option></select></div><div class="full"><label>Letra (opcional)</label><textarea name="lyrics" placeholder="Pegá la letra..."></textarea></div><div class="full"><label>Modelo / Checkpoint ComfyUI</label><div class="inline"><select id="checkpoint" disabled><option value="">Conectá el worker para ver modelos</option></select><button id="refreshModels" class="small" type="button" disabled>Actualizar modelos</button></div><p id="modelHint" class="modelhint">Los modelos se leen automáticamente desde CheckpointLoaderSimple.</p></div><div class="full"><button id="createBtn" class="primary" disabled>Crear proyecto en worker</button></div></form><div id="status" class="status">Conectá el worker para comenzar.</div><div id="preview" class="preview"></div></section><section class="card"><h2>Storyboard</h2><div id="storyboard" class="storyboard"><p class="muted">Creá un proyecto para ver las escenas.</p></div></section></div></main>
<script>
...
async function loadCheckpoints(){
  ...
  cp.disabled=true;refreshModels.disabled=true;cp.innerHTML='<option value="">Cargando modelos…</option>';modelHint.textContent='Consultando CheckpointLoaderSimple en ComfyUI…';
  try{
    const r=await fetch(w('/api/comfyui/checkpoints'),{cache:'no-store'});const j=await r.json();if(!r.ok)throw Error(j.detail||j.error||'No se pudieron leer modelos');
    if(!j.online){resetModels('ComfyUI offline');modelHint.textContent=j.error||'El worker está online pero ComfyUI no responde.';refreshModels.disabled=false;return}
    const models=Array.isArray(j.checkpoints)?j.checkpoints:[];
    if(!models.length){resetModels('No hay checkpoints detectados');modelHint.textContent=j.error||'Colocá un .safetensors/.ckpt en ComfyUI/models/checkpoints y refrescá ComfyUI.';refreshModels.disabled=false;return}
    cp.disabled=false;refreshModels.disabled=false;cp.innerHTML='<option value="">Elegí un modelo…</option>'+models.map(name=>`<option value="${esc(name)}">${esc(name)}</option>`).join('');if(models.length===1)cp.value=models[0];modelHint.textContent=`${models.length} modelo${models.length===1?'':'s'} detectado${models.length===1?'':'s'} en ComfyUI.`;
  }catch(err){resetModels('Error cargando modelos');refreshModels.disabled=false;modelHint.textContent=err.message}
}
async function connectWorker(silent=false){
  const candidate=normalizeWorker(workerInput.value||localStorage.getItem('renderWorkerUrl')||SERVER_WORKER);if(!candidate){WORKER='';badge.textContent='Worker: desconectado';createBtn.disabled=true;resetModels();if(!silent)s.textContent='Pegá la URL HTTPS del worker.';return false}
  workerInput.value=candidate;badge.textContent='Worker: conectando…';connectBtn.disabled=true;
  try{const r=await fetch(candidate+'/api/health',{cache:'no-store'});const j=await r.json();if(!r.ok||!j.ok)throw Error(j.detail||'Health check inválido');WORKER=candidate;localStorage.setItem('renderWorkerUrl',candidate);badge.textContent='Worker: online';createBtn.disabled=false;connectionHint.textContent=`Conectado a ${candidate}`;s.textContent=`Worker online · versión ${j.version||'n/d'}\nCargando modelos de ComfyUI…`;await loadCheckpoints();return true}catch(err){WORKER='';badge.textContent='Worker: offline';createBtn.disabled=true;resetModels('Worker offline');connectionHint.textContent='No se pudo conectar: '+err.message;if(!silent)s.textContent='No se pudo conectar al worker: '+err.message;return false}finally{connectBtn.disabled=false}}
